...
Kakao Local 호출이 인증·쿼터·네트워크 문제로 실패하거나 검색 결과가 없어도 프로그램은 맛집을 `데이터 없음`으로 두고 리포트 생성을 계속합니다. LLM의 1차 JSON 파싱은 실패 시 스키마를 다시 요구해 한 번 재시도합니다. 오류 유형별 처리 정책은 `A1-2.md`의 "오류 처리 정리" 섹션에 정리되어 있습니다.
